...
実際のトーンは自責なので、受け皿は `angry` ではなく `sad`。
...
- ★ **`synthesisTimeoutMs` は2つの場所に効く。** エンジンへの**1リクエストあたり**の上限
  ...
  ★ **どちらもリクエストごとに読み直す。** 応答側を起動時の値で固定すると、
  `PATCH /v1/config` で変えたときに**片方にしか効かない**（200 が返って新しい値が
  エコーされるので、「効いていない」ことに気付けない）
