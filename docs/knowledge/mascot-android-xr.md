...
- **`XRHandSubsystem` は関節を返す。** なので手のひらモードになり、頭上の歯車は出ない
- **エミュレータの手は、手のひらを自分へ向けられない**（向きが固定）。手のひらボタンは出せない
- **つまんでいない間、aim レイは動かない**（マウスを動かしても固定のまま。つまむとその位置へ飛ぶ）。
  aim レイのホバーで出す歯車は、エミュレータでは出せない
- 以上から、**エミュレータではどちらの呼び出し口も出せない。** パネルそのものを確かめるときは、
  歯車を常に出す一時的な改造をしたビルドで確かめる
- `⚙` は `LegacyRuntime.ttf` のフォールバックで描ける

★ **未確認（実機）。** 手のひらを自分へ向けたときにボタンが出るか、つまんでいない間も aim レイが
動くか（歯車のホバー）、パネルの読みやすさ（文字サイズ・行間・距離）。
