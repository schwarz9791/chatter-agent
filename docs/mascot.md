...
**x が負**（画面の左外）で、`screen` も取れない。同じビルドで**出るときと出ないときがある**
——別の日には `frame=3103,2130` で正しく載っていた。

★ **`autosaveName` を付けても直らない**（実測）。位置を決めるのは OS で、押し込む API は無い。
`autosaveName` は付けてあるが、それは**ユーザーが ⌘ドラッグで並べ替えた位置を覚えるため**であって、
この問題の手当てではない。

★ **切り分けはログでできる。** `[Native] ステータスバー: …` が
`item=あり button=あり image=あり visible=1` なら**こちらは仕事を終えている**。
それでも見えないなら、メニューバーの常駐アイコンを減らすか、
Bartender / Ice のようなメニューバー管理ツールを使うしかない。
