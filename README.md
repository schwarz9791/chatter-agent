...
| `apps/chatter-mascot/` | **土台と発話・VRM の表示とも実装済み。** macOS ビルドで透過ウィンドウが成立し、WebSocket → 音声取得 → 再生 → ack が実機で通っています。VRM は表示・待機モーション・視線・表情・まばたき・リップシンクまで入りました。残るはプラットフォーム固有（デスクトップ / XR） |
...
**残っているのはプラットフォーム固有の作り込み**（デスクトップのウィンドウ管理 / XR の Full Space）です。
