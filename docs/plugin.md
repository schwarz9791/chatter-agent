...
> ★ **第1層は実機で効いたことを確認した**（2026-08-17、n=3。詳細は下記
> 「AI要約の実機実測」）。3回の要約すべてで、`spool/` に要約 CLI 由来の delta が一度も現れず、
...
### 所要時間 — `summarizer.log` に記録された3件

`{root}/summarizer.log`（`getSummarizerLogPath`。要約が有効なときだけ書かれる実測ログ）に記録された
3件は、いずれも判定 `ok`（要約が採用された）。

| 原文の長さ | 要約後 | 所要時間 |
|---|---|---|
| 436 文字 | 65 文字 | 15,988 ms |
| 502 文字 | 116 文字 | 21,365 ms |
| 600 文字 | 80 文字 | 26,150 ms |

★ **n=3。傾向であって統計ではない。**

★ **原文の長さと所要時間がきれいに比例している**（436→502 で +5.4秒、502→600 で +4.8秒。おおよそ
100文字あたり 5秒）。この傾きで外挿すると、**700文字前後で `aiSummaryTimeoutMs` の既定 30 秒に届く**。

事前に `claude -p` を単体で叩いた実測（227文字の入力で 10.7〜16.8秒、うち CLI の起動オーバーヘッドが
約5.2秒。→ `docs/core.md` / `CLAUDE.md`）と矛盾しない。今回の3件は原文がそれより長いぶん遅い。

★ **ここから構造的な弱点が見える。** 要約は長い発言ほど効果が大きいのに、長い発言ほど生成に時間が
かかる。**いちばん要約してほしい発言ほど要約が間に合わず、タイムアウトして原文がそのまま読み上げられる**
という逆転が、既定値のまま 700 文字あたりで起きる。
...
★ **ただしこの前提だと、700文字の弱点の意味が変わる。** 目を離して聞いている状況では、
「いちばん長い発言がいちばん長いまま読み上げられる」のはこの用途にとって最も避けたい失敗の仕方になる。
要約が効かないだけでなく、要約を待った20〜30秒のあとに原文の全文が始まる。

`aiSummaryTimeoutMs` を伸ばす／`aiSummaryThreshold` を上げる／要約の指示文を短くして生成を速くする、
といった選択肢はあるが、**まだ検討していない**。実機でも踏んでいない（下記「まだ測っていないこと」）。

### まだ測っていないこと

- 要約が**タイムアウトしたとき**の実挙動（3件とも `ok` で、実機では踏んでいない。原文への
  フォールバックは `verify:phase-a` ⑯とユニットテストでしか確認していない）
- 連続して長い発言をしたときに遅れが積み上がる挙動（`aiSummaryMaxPerDrain` の上限に当たる状況）
- 700文字問題への対策（上の3つの選択肢はいずれも未検討・未実機テスト）

★ **秒数を仕様として扱わないこと。** マシン・ネットワーク・モデルで変わる。上の3件も n=3 の
サンプルであって上限ではない。
