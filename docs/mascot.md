...
**再生中の感情モーションには割り込まない**（最後まで見せる）代わりに、クールダウンで連発を抑える。
クールダウンは**2段**（`MotionParams`）——`CooldownSeconds`（既定 1 秒）はカテゴリを問わない最短間隔、
`SameCategoryCooldownSeconds`（既定 15 秒）は**同じカテゴリ**をもう一度出すまでの間隔。
