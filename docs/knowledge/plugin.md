...
`aiSummaryThreshold` を上げる対策は**効かない**（→ 上記のとおり相関が無いため）。採った対策は
`aiSummaryTimeoutMs` の60秒化と、`summaryPipeline.ts` への要約の長さ上限
（`SUMMARY_MAX_CHARS * 2` = 240文字）の追加（→ `core/src/summarizer/summaryPipeline.ts`）。
...
`LICENSE` / `NOTICE` を `plugin/` 直下にも置いているのは、`/plugin install` が**このディレクトリだけを複製する**ため。
