 ...
 *   bind アドレスをそのまま使うと、LAN 公開のため `0.0.0.0` を指定したときに
 *   `http://0.0.0.0:8570/...` になり、Mac 上のクライアントは偶然繋がるが
 *   **LAN 上の XR グラスからは繋がらない**（`player/client.ts` の `deriveServerUrl` に
 *   同じ罠のコメントがある）。クライアントが自分の WebSocket 接続先の authority に
 *   対して解決する。
 *
