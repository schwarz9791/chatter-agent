 ...
 *
 * 2箇所で使う: 非ループバックにトークン認証を要求する判定（→ `server/auth.ts`）と、
 * 制御 API の書き込み口（`PATCH` / `POST`）を「同じマシンから」に絞る判定
 * （→ `server/controlApi.ts`）。**書き込み口はトークンを持っていてもループバック限定。**
 * トークンは「読ませてよい相手」の証明であって、設定を書き換えさせてよい相手の証明ではない。
 *
