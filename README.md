...
**開発中。macOS の透過ウィンドウに Unity アプリが常駐して、Claude Code の発言を読み上げるところまで動きます。**

| | 状態 |
|---|---|
| `plugin/` | 実装済み。実機で確認済み |
| `core/` | CLI + WebSocket/HTTP サーバー + 発話 CLI + AI要約（既定OFF）+ サーバー合成とも実装済み |
| `apps/chatter-mascot/` | **土台と発話は実装済み。** macOS ビルドで透過ウィンドウが成立し、WebSocket → 音声取得 → 再生 → ack が実機で通っています。VRM の表示はこれから |
...
Phase C は Unity 側の土台まで進んでいます。実機（macOS 26 / Unity 6）で確認できたこと:
...
**残っているのは VRM の表示**（表情・モーション・リップシンク）です。
