...
> ★ **第1層は実機で効いたことを確認した**（2026-08-17、n=10。詳細は下記
> 「AI要約の実機実測」）。10回の要約すべてで、`spool/` に要約 CLI 由来の delta が一度も現れず、
> `speech.jsonl` にも要約 CLI 自身の発言は載らなかった。**第2層は一度も発火していない**
> （捨てるべき payload がそもそも spool に届かなかったため）——実経路での発火は
> `npm run verify:phase-a` の⑰でしか確認できていない。
