...
## 要約と感情判定のバックエンド選定（issue #107）

既定を `claude -p --model haiku` / 辞書式から、`fm`（macOS 27 以降の Apple Foundation Models CLI）と
Ollaya（[ollaya.dev](https://ollaya.dev/)、ローカルの Jev 互換 decision model ランタイム）へ入れ替えた
根拠。**測定条件: M1 Max / 32GB、macOS 27（Darwin 27.0.0）、Ollaya 0.7.1、1台で1件ずつ順番に実行。**

### 要約

`summarizer/prompt.ts` の指示文で、`fm respond -i "<指示文>" --no-stream
--guardrails permissive-content-transformations` を試した（本文は stdin、9件×2回）。

- 速度: 中央値 2.3秒（最小1.7〜最大5.3秒）。同時期の本番 `claude -p --model haiku` の実測
  （n=1954、`summarizer.log`）は中央値17.6秒、タイムアウト41/1954。遅延は入力の長さに比例する
  （900〜1,400字で2.3〜3.3秒、3,100〜3,600字で5.4〜5.7秒）
- `--guardrails permissive-content-transformations` を付けると、ガードレールの誤検知
  （「殺す」「孤児」等の技術用語を安全性理由で拒否する）は0件。口調の指示（原文に近い口語調・
  肯定否定を反転させない・数字はぼかしてよいが違う数字を言い切らない・原文に無い内容を足さない・
  いちばん伝えたい内容を落とさない）を加えても、口調自体は18回中17回で守られた
- **指示文だけでは直らない誤り**: 否定に留保が付いた構文（「遅れなかった。ただし〜可能性は残る」）
  を反転させる誤りが3/3で再発。件数の取り違え・依頼の向きの逆転も各1/18で発生。小さいモデルで
  要約すると事実面がときどきずれる、という前提で受け入れている（要約は既定 ON だが、この代償を
  払ってでも速さを取る、という選択）
- **入力の上限**: 約7,000字（約3,500トークン）を超えると `context size` エラーで失敗する。
  要約対象は整形後の文字数（`aiSummaryThreshold` 既定200字超）なので、実運用のメッセージ
  （多くは200〜900字）はまず当たらない
- 失敗はどちらも終了コード非0・stdout空で、既存の「失敗したら原文」契約にそのまま乗る

### 感情判定

30文（issue記載の15文 + sad/angry らしい10文 + 他の感情がはっきりした5文）を、人手に近いラベル
（Claude が判定基準に沿って付与）と突き合わせた。「厳しめ」は第一候補一致、「緩め」は次点も含む。

| 方式 | 厳しめ | 緩め | 1件あたり（中央値） |
|---|---|---|---|
| `fm`（`--schema` 強制、メッセージ単位） | 20/30 | 26/30 | 2.1秒 |
| Ollaya `decider` の score（英語専用モデル） | 19/30 | 23/30 | 8.3秒 |
| Ollaya `decider` の choice | 18/30 | 23/30 | 2.6秒 |
| Ollaya `laya:multilingual` の score（英語の指示） | 16/30 | 21/30 | 0.06秒 |
| Ollaya `laya:multilingual` の choice（英語の指示） | 14/30 | 20/30 | 0.03秒 |
| 辞書式（従来） | 12/30 | 18/30 | — |
| Ollaya `laya` の choice（日本語の指示） | 9/30 | 15/30 | 0.03秒 |
| 参考: haiku 単発（15文中、ラベルが出た13文） | 9/13 | 11/13 | 9.4秒 |

- **`laya` は日本語の指示だと精度が落ちる**（完了報告を sad と判定する等）。英語に書き換えると
  改善したが、本文は日本語のまま渡している。`laya:en` に日本語の本文を読ませるとどの形でも悪化した
- **`decider` は英語専用モデル**（`ollaya show` の表示上も）で、推論中に8.5〜10GBを使う。重すぎるため
  既定にはしていない
- `fm` はスキーマ強制で `neutral` への偏りが大きく減った（30文中2文のみ）が、`surprised` に寄る傾向
  （「バグが見つかった」系の文を surprised 1.0 にしがち）が残る。同じ15文を2回回すと4文でラベルが
  変わる（決定的ではない）
- **メッセージ単位（1回）でまとめて判定する方式を採った理由**: `fm` を1文ずつ判定すると1文2秒台
  かかり、10文のメッセージで20秒になる。`fm` のスキーマ強制バッチ（10文をまとめて1回）は15.0秒
  （1文あたり1.5秒）で収まったが、30文のバッチは context size 超過で失敗した。**`fm` はメッセージ
...
- `ollaya serve` に `--host` / `--port` オプションは無い（`--help` にも出ない）。バイナリの文字列に
  `OLLAYA_HOST`（`host:port` 形式）があり、`ollaya serve` 自身のエラーメッセージ
  （ポート使用中のとき）にも `OLLAYA_HOST=127.0.0.1:<port> ollaya serve` という案内が出る。
  これで既定ポート以外でも起こせる（`server/engineProcess.ts` の `resolveOllayaSpawn`）
...
### `pipe()` ではなく `pipeline()` を使う

`GET /v1/assets/<path>`（`server/httpServer.ts` の `handleAsset`）は `fs.createReadStream()` を
レスポンスへ流す。**`pipe()` だと宛先（`res`）が閉じても読み出し側のストリームは開いたままになる。**
この経路は Range による再開を前提にしていて**途中で切られるのが普通に起きる**ので、漏れれば
ファイルディスクリプタが溜まり続け、常駐プロセスの寿命に直結する。`pipeline()` に替えると
両側が確実に閉じる。`verify-assets.mjs` の⑥（中断を20回繰り返してからの疎通確認）がこの回帰を見る。

### `fetch()` の URL パーサは送信前に `..` を畳む

`verify-assets.mjs` のトラバーサル検査で踏んだ。`fetch("http://h/v1/assets/../../etc/passwd")` は
送信前にパスが正規化されてしまい、サーバーには畳んだ後の別のパスが届く——この書き方では
「サーバーがトラバーサルを拒否しているか」を検査したことにならない。`%2e%2e` のような
