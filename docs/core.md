...
加えて、設計書に無い挙動を1つ足した。**保留している最後の文を、後続イベントの到着で先に流す。**
`final:true` を待つと 34〜80 秒遅れるが、後続イベントが来た時点でそのメッセージはもう伸びない。
順序を保ったまま遅延だけを消せる。
...
移植した `cleanTextForSpeech` は強調（`**` / `*` / `__` / `~~`）とリンク記法を扱わない。上流にも
これを保持する意図のテストは無く、単なる未対応。TTS にそのまま渡ると読み上げに混ざる
（`**` だけの行が1発話になることもあり、リンクは `[text](` という残骸になる）。
