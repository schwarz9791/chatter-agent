...
| `apps/chatter-mascot/` | **土台と発話・VRM の表示とも実装済み。** macOS ビルドで透過ウィンドウが成立し、WebSocket → 音声取得 → 再生 → ack が実機で通っています。VRM は表示・待機モーション・視線・表情・まばたき・リップシンクまで入りました。**ウィンドウの位置と大きさも覚えます**（ディスプレイをまたいでも物理サイズが変わらず、画面外に置いても次の起動で拾い直す）。**デスクトップ常駐（Dock 非表示・メニューバー・ミュート）と設定 UI も入りました** —— キャラクターを右クリックすると設定パネルが開き（もう一度で閉じます）、モデル・大きさ・音量・話す速さ・音声スタイル・モーション・要約・ショートカットを変えられます。「大きさ」はウィンドウごと変わるので、大きくしても頭が欠けません。**感情に合わせたモーションと、待機中の小ネタも入りました**（モーションは `~/.config/chatter-agent/animations/<感情>/` に `.vrma` を置くと増やせます）。表情の出方が合わないときは `~/.config/chatter-agent/emotion-keywords.json` を編集すると調整でき、ファイルを消せば既定に戻ります。**Android（XR なし）のビルドも通り**、Android XR エミュレータで接続・発話・VRM の表示まで確認しました。残るは XR そのもの |
...
- **発言の感情でワンショットのモーションが出て、待機へ滑らかに戻る**

**残っているのは XR 固有の作り込み**（Full Space / 接続先の設定 / 実機確認）です。Android のビルドと、エミュレータでの接続・発話は通っています。デスクトップ側は、ウィンドウの位置と大きさ・常駐・設定 UI まで揃っています。
