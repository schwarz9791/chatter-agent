...
| `core/` | `chatter-agent-core`（CLI + WebSocket/HTTP サーバー + 発話 CLI） | **実装済み。** `summarizer/`（AI要約、既定OFF）・**サーバー合成**（[#29](https://github.com/schwarz9791/chatter-agent/issues/29)）・**エンジンの spawn**（[#51](https://github.com/schwarz9791/chatter-agent/issues/51)）・**設定パネルの制御 API**（[#76](https://github.com/schwarz9791/chatter-agent/issues/76)。`/v1/*`。**書き込み口はループバック限定**）も含めて完了 |
