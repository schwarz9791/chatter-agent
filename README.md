...
Phase A は実機で動作確認しました。delta が hook に届いてから `speech.jsonl` に載るまでの配管は**約 50ms** です。発話は**メッセージが閉じた（`final:true`）タイミングで全文をまとめて**流します（実測では複数文のメッセージ 179件のうち 97.8% で即座に届きます）。**あとは VRM で表示する側（`apps/`）だけが残っています。**
