...
2本目を選ぶ（`aaa-decoy.vrm` は無傷）/ 位置と大きさのリセット / すべての設定をリセット（`NSAlert` の
destructive 確認）/ 「Chatter Mascot について」/ 設定パネルの項目 / 話者一覧 / スライダーの刻み /
テスト音声 / サーバーを止めて開く / 再起動しても残る）も実機確認、全項目パス。
...
## ★ XR の設定パネル（#143）はデスクトップと同じスキーマを使う

`SettingsSchema.Build` は唯一の入口のまま、`SettingsContext.Platform`（`Desktop` / `Xr`）で
出し分ける。**デスクトップの出力は一切変えない**——`Build` の分岐は `Platform` だけを見ることに
限り、XR 側の都合でデスクトップの並びに手を入れない。実装は
[`mascot-android-xr.md`](./mascot-android-xr.md)「空間に浮かぶ設定パネル」。

**「すべての設定をリセット」の確認は、ダイアログではなく「もう一度押す」で行う。** デスクトップは
`NSAlert` の destructive 確認（→ 上「#76 の実機確認」）を持てるが、**XR にはネイティブの
確認ダイアログが無い**——1回押しただけで確定させると誤操作で全部戻る。パネル側に確認用の
