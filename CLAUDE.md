...
**要約 ON での実機確認も行った**（Claude Code 2.1.233 / macOS、n=3。詳細は [`docs/plugin.md`](./docs/plugin.md)）。
無限ループ防止の第1層は3件とも効いていて、原文の長さと所要時間はほぼ比例し（おおよそ100文字あたり5秒）、
この傾きで外挿すると**700文字前後で `aiSummaryTimeoutMs` の既定30秒に届く**——要約がいちばん効くはずの
長い発言ほど要約が間に合わずタイムアウトし、原文がそのまま読み上げられるという逆転が既定値のまま起きる。
秒数はマシンとネットワークで変わるので**仕様として扱わないこと**。
...
>
> ★ **ただしこの前提だと、700文字の弱点の意味が変わる。** 目を離して聞いている状況では、要約待ちの
> 20〜30秒のあとに**いちばん長い発言がいちばん長いまま**読み上げられるのが、いちばん避けたい失敗の
> 仕方になる。`aiSummaryTimeoutMs` を伸ばす／`aiSummaryThreshold` を上げる／要約の指示文を短くする
> といった選択肢はあるが、**まだ検討していない**。タイムアウトの実挙動そのものも実機では未確認
> （3件とも `ok` で終わった）。詳細と残りの未検証事項は [`docs/plugin.md`](./docs/plugin.md)。
