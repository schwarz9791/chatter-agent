...
describe("読み上げたくない末尾は捨てる", () => {
  ...
  it("★ [#32] 未閉じの表の行がメッセージ末尾の改行の手前にあっても生パイプを読み上げない", () => {
    // 再現: メッセージが改行で終わる（unstableTail.test.ts の「最後の行が空文字になる」バグ）
    expect(speak(["手順です。\n| C | D\n"])).toEqual(["手順です。"]);
  });

  it("★ [#32] 未閉じの表の行が最終行でなくても検出し、その先の文は失う（代償込みの回帰）", () => {
    expect(speak(["手順です。\n| A | B\n完了です。"])).toEqual(["手順です。"]);
  });

  it("★ [#32] 地の文の迷子フェンスは後続の文を巻き込まない", () => {
    // 修正前は地の文の ``` で開閉が反転し、「を使います。おわりです。」が無音で消えていた。
    // 対になる相手が見つからない ``` は cleanTextForSpeech でも除去されず文字として残るが、
    // 読み上げエンジンは記号を発音しないので実害は小さい（CLAUDE.md 実測ノート）。
    const total = speak(["説明します。\n```\ncode\n```\n以上です。バッククォート ``` を使います。おわりです。"]);
    expect(total).toEqual(["説明します。", "以上です。", "バッククォート ``` を使います。", "おわりです。"]);
    expect(total.join("")).not.toContain("code");
  });
});
