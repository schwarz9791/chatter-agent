...
| まばたき | |
| 位置をリセット | `ModelAnchor` を起動時の位置・向きへ戻し、歩行範囲の半径も既定（身長 25cm のとき 20cm）へ戻す |
...
手でつまむとキャラクターを動かせる。マウス（Android XR に繋いだマウス／トラックパッド）の
クリック・ドラッグでも、つまむのと同じ操作ができる。離すと真下の水平面（机・床）に足元が乗り、
こちらを向く。**平面に乗せた直後だけ、足元に歩行範囲の円が 15 秒出る。** フチのハンドルを
つまんで前後に動かすと半径が変わる（身長 25cm のとき 10〜60cm、既定 20cm。実際の広さは
キャラクターの大きさに比例して伸び縮みする）。キャラクターは発話が止まってからしばらくすると
この円の中をたまに歩き、**円の外へは出ない**。

- **円の中をつまむと、そこへ歩いて行く**（ハンドルにも体にも当たらないつまみ。足元に当たったつまみも床として扱う）
- 体をつまんで動かさずに離すと、その場で円を出し直す
- 歩くには `animations/walk/` に `.vrma` が要る（→ 上「感情モーションと小ネタの素材」）。無ければ歩かない
- **平面に置き直すまでは歩かない**（起動直後は床が分からないため）。平面の無い所で離したときも、次に平面へ置くまで歩かない
- 発話中・つまんでいる間は歩かない
- **つまんで置き直しても、変えた半径は保たれる。** 既定（身長 25cm のとき 20cm）へ戻すのはパネルの「位置をリセット」だけ
