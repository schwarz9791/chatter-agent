...
キャラクターの近くに**空間に浮かぶ設定パネル**を出せる。呼び出し方は2通りで、毎フレーム
「手の関節が取れているか」だけで自動的に切り替わる（機種名では判定しない）。

- **手の関節が取れている**: 手のひらを自分（頭）の方へ向けると、その手のひらの上にボタンが出る。
  もう片方の手でつまむと開く
- **取れていない**: キャラクターの頭上に歯車が出る（aim レイをキャラクターか歯車に当てている
  間だけ。当て続けなくても少しの間は残る）。つまむと開く
