...
| `apps/chatter-mascot/` | **土台と発話・VRM の表示とも実装済み。** macOS ビルドで透過ウィンドウが成立し、WebSocket → 音声取得 → 再生 → ack が実機で通っています。VRM は表示・待機モーション・視線・表情・まばたき・リップシンクまで入りました。**ウィンドウの位置と大きさも覚えます**（ディスプレイをまたいでも物理サイズが変わらず、画面外に置いても次の起動で拾い直す）。**デスクトップ常駐（Dock 非表示・メニューバー・ミュート）と設定 UI も入りました** —— キャラクターを右クリックすると設定パネルが開き、モデル・大きさ・音量・話す速さ・音声スタイル・モーション・要約・ショートカットを変えられます。残るは XR |
