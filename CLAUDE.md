...
- `final:true` の 34〜80 秒遅延は再現しなかったが、**`AskUserQuestion` の直前という条件を踏んでいないだけの可能性がある**
...
> 設計書 §2-4 は 2.1.231 で「最終チャンクだけが 34〜80秒遅れて届く」と実測している。2.1.233 の計測では再現しなかった（ツール呼び出しの直前で +3.3〜7.0 秒、ターン終了で +0.62 秒）が、**`AskUserQuestion` / `ExitPlanMode` の直前という条件を踏んでいないだけの可能性がある**（→ [`docs/plugin.md`](./docs/plugin.md)）。いずれにせよ**待たない設計をやめる理由にはならない。**
