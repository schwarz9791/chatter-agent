...
| `connection.assetSync` | 効く（設定パネルの「モデルとモーションをサーバーから受け取る」からも変えられるが、**次回の起動から**——読むのは起動時の1回だけ） |
...
- エミュレータで `XRHandSubsystem` が実際に関節を返すか（返さなければ手のひらメニューは常に
  歯車へ落ちる。実機での確認が要る）
- つまんでいない間も Hand Interaction Profile の `isTracked` が `true` のままか（落ちるなら
  aim レイでのホバーが取れず、頭上の歯車が出ない）
- 歯車・手のひらボタンの `⚙` が `LegacyRuntime.ttf` のフォールバックグリフで実際に描けるか
- パネルの見た目・読みやすさ（文字サイズ・行間・距離）が実際のグラスで妥当か
